...
function ReviewPage() {
  ...
  // Persist this completed interview so it shows up in the candidate's
  // history page and in the recruiter dashboard. This is best-effort:
  // if it fails, the person still sees their review, they just won't
  // have this attempt saved to their history.
  const saveInterviewToHistory = async (
    reviewText,
    reviewScore,
    reviewCategoryScores,
  ) => {
    if (hasSaved || !gToken) return;
    try {
      setHasSaved(true);
      await axios.post(
        `${serverURL}/api/interview/save`,
        {
          job_role: gJobRole,
          exp_level: gJobExp,
          qns: gQtns,
      ...
      console.log(
        "Could not save this interview to history:",
        error.response ? error.response.data.errorMsg : error.message || error,
      );
    }
  };
